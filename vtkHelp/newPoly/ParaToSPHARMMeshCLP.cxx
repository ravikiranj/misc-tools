  ...
  try
    ...
    // save associated surface
	 ...
	 if(medialMesh)
	 {
        ...
        //Create new data
        vtkPolyData* newPolyData = ITKVTKConverter5->GetOutput();
        double *m_PointRadius = medialmeshsrc->GetPointRadius();
        double *m_PointAvgRadius = medialmeshsrc->GetPointAvgRadius();
        long int totalPoints = thetaIteration * phiIteration;

	    vtkSmartPointer<vtkFloatArray> PointRadius = vtkSmartPointer<vtkFloatArray>::New();
    	vtkSmartPointer<vtkFloatArray> PointAvgRadius = vtkSmartPointer<vtkFloatArray>::New();

        PointRadius->SetName("PointRadius");
        PointRadius->SetNumberOfTuples(totalPoints);
        PointAvgRadius->SetName("PointAvgRadius");
        PointAvgRadius->SetNumberOfTuples(totalPoints);
        for(int i = 0; i < totalPoints; i++){
            PointRadius->SetTuple1(i, m_PointRadius[i]);
            PointAvgRadius->SetTuple1(i, m_PointAvgRadius[i]);
        }
        /*
        PointRadius->SetName("PointRadius");
        PointRadius->SetNumberOfComponents(totalPoints);
        PointAvgRadius->SetName("PointAvgRadius");
        PointAvgRadius->SetNumberOfComponents(totalPoints);

        for(int i = 0; i < totalPoints; i++){
            PointRadius->InsertComponent(0, i, m_PointRadius[i]);
            PointAvgRadius->InsertComponent(0, i, m_PointAvgRadius[i]);
        }
        */

    	newPolyData->GetPointData()->AddArray(PointRadius);
    	newPolyData->GetPointData()->AddArray(PointAvgRadius);
        newPolyData->GetPointData()->SetScalars(PointRadius);

        vtkwriter->SetInput(newPolyData);
        outFileName.erase();
        outFileName.append(base_string);
        outFileName.append("NEWPoly.vtk");
		vtkwriter->SetFileName(outFileName.c_str() );
		vtkwriter->Write();
	 }
